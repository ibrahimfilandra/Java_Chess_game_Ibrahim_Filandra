...
public class Board implements Cloneable {
    ...
    void move(Class type, ChessPiece.Color color, String position) {
        ...
        String pomocna = "";
        int rednadjene = 0, kolonanadjene = 0;
        prva:
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
        ...
        if (brojaczafigure == brojaczaizuzetke)
            throw new IllegalChessMoveException(); //ako za svaku provjerenu figuru je bacilo izuzetak da je potez ilegalan
        ...
        if (!(board[indeksiraj(position.charAt(1))][position.charAt(0) - 65] instanceof PraznoPolje) && board[indeksiraj(position.charAt(1))][position.charAt(0) - 65].getColor() == color)
            ...
            int red = indeksiraj(position.charAt(1));
            int kolona = position.charAt(0) - 65; //koordinate odredisnog polja u 2d nizu
    ...
    void move(String oldPosition, String newPosition) {
        ...
        if (board[rednadjene][kolonanadjene] instanceof PraznoPolje)
            throw new IllegalArgumentException(); //ako na oldPosition nema figure
        try {
